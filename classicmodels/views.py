...
from .filters import FilterBackendMixin, CustomSearchFilter
...
class CustomerAPIView(generics.ListAPIView, FilterBackendMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        filterset_class = self.dynamic_model_filter_set(meta_model=Customers, fields=[],
                                                        request=self.request, filter_fp=filter_fp)
        ...
        if sort_by:
            queryset = self.dynamic_ordering(queryset=queryset, sort_by=sort_by, order_by=order,
                                             annotate_fields=["first_name", "last_name"])
